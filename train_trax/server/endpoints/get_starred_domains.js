...
const endpoint = (request, response) => {
	...
	server.query('SELECT * FROM users WHERE user_token = $1;', [user_token], (error, res) => {
        ...
        if(is_library)
        {
            server.query('SELECT * FROM starred_articles WHERE is_domain = TRUE;', [user_data.user_id], (error, res) => {
                if(error) throw error;
            });

            for(let row of res.rows) domain_data.push(row.article);
        }
        else
        {
            server.query('SELECT * FROM starred_articles WHERE user_id = $1 AND is_domain = TRUE;', [user_data.user_id], (error, res) => {
                if(error) throw error;
            });
